  ...
  keymaps = [
    {
      mode = ["n"];
      key = "<Leader>da";
      action = lib.nixvim.mkRaw ''function() require("dap").continue({ before = get_args }) end'';
    ...
    }
    {
      mode = ["n"];
      key = "<Leader>dB";
      action = lib.nixvim.mkRaw ''function() require("dap").toggle_breakpoint(vim.fn.input("Breakpoint condition: ")) end'';
    ...
    }
    {
      mode = ["n"];
      key = "<Leader>?";
      action = lib.nixvim.mkRaw ''function() require("dapui").eval(nil, { enter = true }) end'';
